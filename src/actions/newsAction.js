...
import { NEWS_LIST_REQUEST, NEWS_LIST_FAIL, NEWS_LIST_SUCCESS, NEWS_DETAILS_REQUEST, NEWS_DETAILS_FAIL, NEWS_DETAILS_SUCCESS, NEWS_ADD_REQUEST, NEWS_ADD_FAIL, NEWS_ADD_SUCCESS, NEWS_DELETE_REQUEST, NEWS_DELETE_SUCCESS, NEWS_DELETE_FAIL } from "../constants/newsConstant";

export const newsCollections = ({ link = '' }) => async (dispatch) => {
    dispatch({ type: NEWS_LIST_REQUEST })
    try {
        const { data } = await Axios.get(`/api/news?link=${link}`)
...
export const newsDetails = (productId) => async (dispatch) => {
    ...
    try {
        const { data } = await Axios.get(`/api/news/${productId}`)
...
export const addNews = (item) => async (dispatch, getState) => {
    ...
    try {
        const { data } = await Axios.post('/api/news/addnews', item, {
